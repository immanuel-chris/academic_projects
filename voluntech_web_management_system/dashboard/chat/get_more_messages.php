...
if (isset($_GET['id']) && $_GET['id'] != -1) {
    ...
    $sqlIndivChat = "SELECT * FROM messages WHERE isorgchat=0 AND (username='".$rowFromIdResult['username']."' AND receiver='".getUsernameFromId($_GET['id'], $conn)."') OR (username='".getUsernameFromId($_GET['id'], $conn)."' AND receiver='".$rowFromIdResult['username']."') ORDER BY datesent DESC LIMIT 8 OFFSET ".$_GET['offset'];
    $resultMessageHist = mysqli_query($conn, $sqlIndivChat);
}
else {
    $sqlGroupChat = "SELECT * FROM messages WHERE isorgchat=1 ORDER BY datesent DESC LIMIT 8 OFFSET ".$_GET['offset'];
...
echo json_encode(["contents" => $hist ]);
exit();